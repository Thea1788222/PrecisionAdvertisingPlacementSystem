...
1. **第一层识别：Cookie**
   ...
   - 在广告追踪服务域名下设置第三方Cookie
   - Cookie名称：`ad_uid`
   - 有效期：365天
   - 作用域：`.example.com`（跨子域名共享）
...
| 技术        | 版本  | 用途    | 选型理由      |
| --------- | --- | ----- | --------- |
| Bootstrap | 5.3 | UI框架  | 快速开发响应式界面 |
| Video.js  | 8.x | 视频播放器 | 功能强大、易扩展  |
| Chart.js  | 4.x | 数据图表  | 轻量级图表库    |
| vue       | 3.x | 前端框架  | 开发便捷      |
...
- [ ] 使用Spring Initializr创建5个项目
  - ad-admin（广告管理平台）
  - ad-tracker（广告追踪服务）
  - shop-site（购物网站）
  - video-site（视频网站）
  - news-site（新闻网站）
- [ ] 配置application.yml（数据库连接等）
- [ ] 创建Entity实体类
- [ ] 创建Repository接口
- [ ] 编写通用工具类
...
- [ ] 编写启动脚本
  ...
  ```bash
  # start.shnohup java -jar ad-tracker.jar --server.port=8081 > tracker.log 2>&1 &
  ```
...
server {
    listen 80;
    server_name ad-track.example.com;
