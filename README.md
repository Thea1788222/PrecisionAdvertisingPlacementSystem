...
1. **AdImpressionServiceImpl** - 广告展示与点击追踪
   - 记录广告展示信息，包括广告ID、用户指纹、网站、位置和出价
   - 更新广告点击状态，实现展示与点击的关联

2. **AdRecommendationServiceImpl** - 智能广告推荐
   - 根据用户行为分决定推荐策略
   - 高行为分用户：按兴趣权重分配80%，随机20%
   - 低行为分用户：权重最高30% + 权重最低20% + 随机50%
   - 新用户：随机推荐

3. **UserBehaviorServiceImpl** - 用户行为处理
   - 保存用户行为数据到Redis队列，等待批量处理
   - 定时批量处理用户行为数据，更新用户画像
   - 计算行为得分和时间衰减因子
   - 合并新旧兴趣权重

4. **UserProfileServiceImpl** - 用户画像管理
   - 根据用户指纹获取用户画像
   - 解析兴趣JSON字符串为Map
   - 解析类别JSON字符串为List
   - 获取用户最高权重的兴趣类别
...
- 浏览行为：基础分1 + 时长加权分（每10秒加0.5分，上限5分）
- 点击行为：固定加分1
- 时间衰减：使用指数衰减函数 e^(-λt)，λ=0.1

**数据处理策略**

- 使用Redis队列缓存用户行为数据，避免高并发下的数据库压力
- 定时批量处理用户行为数据（每分钟执行一次）
- 按用户指纹分组处理，提高处理效率
